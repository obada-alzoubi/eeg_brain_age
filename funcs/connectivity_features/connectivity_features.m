...
switch feat_name
  ...
  case {'connectivity_coh_mean','connectivity_coh_max','connectivity_coh_freqmax'}
    ...
    fprintf('Modified Coh for F3 and  P3 only \n')
    N_pairs=size(ipairs,2);

    ipairs= {{3}, {4}, {3}, {7}, {4},{8},{7}, {8}, {3}, {27}, {4}, {28},...
        {27}, {28}, {3}, {8}, {4},{7},{3},{28},{4},{27} };
    
    N_pairs=size(ipairs,2)/2;
    %N_pairs = s
    ...
    for p=1:N_pairs        
        %x1=x(ipairs(1,p),:);
        %x2=x(ipairs(2,p),:);
        p1_ind = cell2mat(ipairs{1, (p-1)*2 + 1});
        p2_ind = cell2mat(ipairs{1, (p-1)*2 + 2});
